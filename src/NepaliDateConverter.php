...
class NepaliDateConverter
{
    ...
    public static function totalDaysSince1970(int $y, int $m, int $d): int
    {
        ...
        for ($Y = self::$startYear; $Y < $y; $Y++) {
            if (!isset(self::$bsData[$Y])) throw new \Exception("Year $Y not in dataset");
    ...
    {
        [$by, $bm, $bd] = array_map('intval', explode('-', self::normalize($bsDate)));
        if (!isset(self::$bsData[$by])) throw new \Exception("BS Year out of range");
    ...
    {
        if (!preg_match('/^\d{4}-\d{2}-\d{2}$/', $bsDate)) return false;
        [$y, $m, $d] = array_map('intval', explode('-', $bsDate));
        if (!isset(self::$bsData[$y])) return false;
        if ($m < 1 || $m > 12) return false;
        if ($d < 1 || $d > self::$bsData[$y][$m - 1]) return false;
    ...
    {
        if (!preg_match('/^\d{4}-\d{2}-\d{2}$/', $adDate)) return false;
    ...
    {
        if (!self::isValidBSDate($bsDate)) return [];
    ...
    {
        if (!self::isValidADDate($adDate)) return [];
